...
#define			  	SENSOR_DATA_ARRAY_SIZE 1 * 16
...
#define			    ONBOARD_SENSOR_DATA_ARRAY_SIZE 2 * 16
...
void Sensor_Read_Init(void)
{
	...
	Sensor_LSM6DSOX_rb.p_i2c			= &Onboard_Sensor_I2C;
	Sensor_LSM6DSOX_rb.p_request_buffer	= g_onboard_sensor_H3LIS331DL_request_buffer;
...
static uint8_t sensor_I2C_error_handle(sensor_request_rb_t* p_current_sensor, i2c_result_t* p_return_value)
{
	...
	*p_return_value = false;
	...
	case I2C_ERROR_SENSOR_NOT_CONNECTED:
	{
		...
		sprintf(msg, "> ERROR: SENSOR %s IS NOT RESPONDING ON I2C BUS (NO ACK)\n", p_current_sensor->p_sensor_name);
	...
	case I2C_ERROR_BUS_ERROR:
	{
		...
		sprintf(msg,"> ERROR: SENSOR %s I2C BUS ERROR (SIGNAL ERROR OR MISWIRED)\n", p_current_sensor->p_sensor_name);
		UART_Driver_SendString(CMD_line_handle,msg);
	...
	case I2C_ERROR_BUS_BUSY:
	{
		...
		sprintf(msg, "> ERROR: SENSOR %s I2C BUS CURRENTLY BUSY (SIGNAL ERROR OR MISWIRED)\n", p_current_sensor->p_sensor_name);
		UART_Driver_SendString(CMD_line_handle,msg);
	...
	case I2C_FAIL:
	{
		...
		sprintf(msg,  "> ERROR: INTERNAL ERROR, PLEASE RESET\n", p_current_sensor->p_sensor_name);
		UART_Driver_SendString(CMD_line_handle,msg);
