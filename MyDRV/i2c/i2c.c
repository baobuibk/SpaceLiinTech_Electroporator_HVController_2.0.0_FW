...
I2C_Status_t I2C_Write(I2C_Handle_t *hi2c, uint8_t addr, uint8_t reg, uint8_t data)
{
    ...
    timeout = I2C_TIMEOUT;
    while (!LL_I2C_IsActiveFlag_ADDR(hi2c->Instance) && timeout--);
    if (timeout == 0) return I2C_Error;
    LL_I2C_ClearFlag_ADDR(hi2c->Instance); // Phai xoa co ADDR de tiep tuc

    // 4. Gui dia chi thanh ghi (reg)
    timeout = I2C_TIMEOUT;
    while (!LL_I2C_IsActiveFlag_TXE(hi2c->Instance) && timeout--);
    if (timeout == 0) return I2C_Error;
    ...
    timeout = I2C_TIMEOUT;
    while (!LL_I2C_IsActiveFlag_BTF(hi2c->Instance) && timeout--);
    if (timeout == 0) return I2C_Error;
...
I2C_Status_t I2C_Read(I2C_Handle_t *hi2c, uint8_t addr, uint8_t reg, uint8_t* pData)
{
    ...
    timeout = I2C_TIMEOUT;
    while (!LL_I2C_IsActiveFlag_RXNE(hi2c->Instance) && timeout--);
    if (timeout == 0) return I2C_Error;
...
void I2C_EV_IRQHandler(I2C_Handle_t *hi2c)
{
    ...
    if (LL_I2C_IsActiveFlag_SB(I2Cx)) {
        if (hi2c->State == I2C_STATE_RX_MEM_ADDR || hi2c->State == I2C_STATE_TX_MEM_ADDR) {
            LL_I2C_TransmitData8(I2Cx, (hi2c->DevAddress << 1)); // Ghi địa chỉ slave + Write
        } else if (hi2c->State == I2C_STATE_RX_RESTART) {
            LL_I2C_TransmitData8(I2Cx, (hi2c->DevAddress << 1) | 1); // Ghi địa chỉ slave + Read
    ...
    if (LL_I2C_IsActiveFlag_ADDR(I2Cx)) {
        ...
        if (hi2c->State == I2C_STATE_RX_DATA) {
            if (hi2c->Count == 1) {
                // Đọc 1 byte: Phải NACK và STOP ngay lập tức khi xóa ADDR
                LL_I2C_AcknowledgeNextData(I2Cx, LL_I2C_NACK);
                LL_I2C_ClearFlag_ADDR(I2Cx);
                LL_I2C_GenerateStopCondition(I2Cx);
            }
            else if (hi2c->Count == 2) {
                // Đọc 2 byte: Đòi hỏi đặc biệt - NACK và POS phải set trước khi xóa ADDR
                LL_I2C_AcknowledgeNextData(I2Cx, LL_I2C_NACK);
                I2Cx->CR1 |= I2C_CR1_POS; // Bật cờ POS
                LL_I2C_ClearFlag_ADDR(I2Cx);
            }
            else {
                // Đọc > 2 byte
            ...
            LL_I2C_ClearFlag_ADDR(I2Cx);
            if (hi2c->State == I2C_STATE_RX_MEM_ADDR || hi2c->State == I2C_STATE_TX_MEM_ADDR) {
                LL_I2C_TransmitData8(I2Cx, hi2c->MemAddress); // Gửi địa chỉ thanh ghi
            }
        }
    }
    ...
    if (LL_I2C_IsActiveFlag_BTF(I2Cx)) {
        if (hi2c->State == I2C_STATE_RX_MEM_ADDR) {
            // Chờ BTF (truyền xong MemAddress) mới tạo Repeated Start
            hi2c->State = I2C_STATE_RX_RESTART;
            LL_I2C_GenerateStartCondition(I2Cx);
        }
        else if (hi2c->State == I2C_STATE_TX_DATA && hi2c->Count == 0) {
            // Truyền hết data và BTF set -> Tạo STOP
            LL_I2C_GenerateStopCondition(I2Cx);
            ...
            // Trường hợp đọc đa byte có dùng BTF (Count == 2 và Count > 2)
            if (hi2c->Count == 2) {
                // Đợi BTF set khi còn đúng 2 byte -> tạo STOP và NACK
                LL_I2C_GenerateStopCondition(I2Cx);
                ...
                *hi2c->pBuffer++ = LL_I2C_ReceiveData8(I2Cx);
                ...
                // Khi còn 3 byte, xóa ACK để báo NACK cho byte cuối
                LL_I2C_AcknowledgeNextData(I2Cx, LL_I2C_NACK);
                *hi2c->pBuffer++ = LL_I2C_ReceiveData8(I2Cx);
                hi2c->Count--;
            }
        ...
        if (hi2c->State == I2C_STATE_RX_DATA) {
            if (hi2c->Count > 3) {
                ...
                *hi2c->pBuffer++ = LL_I2C_ReceiveData8(I2Cx);
                ...
                I2Cx->CR1 &= ~I2C_CR1_POS; // Tắt POS nếu có bật
